...
fun main() {
    ...
    val usuarios = mutableListOf(usuario1, usuario2)
    ...
    usuarios.forEach { 
        usuario ->print("${usuario.nome} está cursando o ")
    ...
    val usuario3 = Usuario("Alexandre", listOf(cursoAvancado), listOf(modulos[4])) 
    usuarios.add(usuario3)
    val usuario4 = Usuario("Marcia", listOf(cursoIntermediario), listOf(modulos[1])) 
    usuarios.add(usuario4)
    
    //Atualizando, agora Maria está no modulo 2
    usuarios[1]= Usuario("Maria", listOf(cursoIntermediario), listOf(modulos[1]))
